...
class SessionManager {
  ...
  async attachmentUrl(fileName) {
    return await this.driver.attachmentUrl(this.session, fileName);
  }
  ...
  async save(form) {
    if (window.WebViewBridge && window.WebViewBridge.beforeSave) {
      form = window.WebViewBridge.beforeSave(form);
    }
